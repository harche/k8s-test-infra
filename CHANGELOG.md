...
- node-agent: new `rdmaplugin` simulator makes mock nodes advertise the
  ...
  `.status.capacity` on `Apply`, re-asserting every 30s, and withdrawing the key
  on shutdown, since kubelet never removes capacity it does not own. The count
  is declared per profile (`infiniband.rdma_resource`, `rdma/ib: 64` on all five
  IB-enabled profiles); upstream that count is `rdmaHcaMax`, a pod-sharing limit
  rather than an HCA count, so a 4-HCA GB300 compute tray reports 64. Needs the
