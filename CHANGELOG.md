...
- The Tilt environment's default `--gpu-profile` is now `gb300` instead of
  `a100`. Ada-and-later profiles report the slowdown T.Limit thermal margin that
  the thermal-margin consumers key on, so the default no longer silently
  excludes them; pass `--gpu-profile a100` to get the previous shape. The
  heterogeneous `--multi-gpu-profile` fleet is unaffected and stays a100 + t4.
