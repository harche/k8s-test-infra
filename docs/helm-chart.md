...
which advertises `rdma/ib` in the node's capacity. No such plugin runs against
the mock, so the node agent's `rdmaplugin` simulator stands in for it: it
patches the key into `.status.capacity`, re-asserts it every 30s so an
overwrite by another controller cannot silently drop it, and withdraws it on
shutdown. Kubelet mirrors the key into `allocatable` on its next status sync.

`hca_max` is that plugin's `rdmaHcaMax` — how many pods may concurrently share
the node's HCAs — so it is unrelated to `hcas_per_gpu`. A 4-HCA GB300 compute
tray reports `rdma/ib: 64`, matching real hardware.

All five IB-enabled profiles ship `rdma/ib: 64`. Pods requesting the resource
...
the failure, retries on each re-assertion tick, and pods requesting the resource
stay `Pending`.
