...
| `rdmaplugin`            | Component 5's Kubernetes face — RDMA shared device plugin       | resolve + validate the profile's `rdma_resource`                                            | `rdma/ib` in the node's `status.capacity` | re-assert the capacity key every 30 s                              |
...
```
   ...
   State snapshot ───┼── imex           (chardevs + /proc/devices overlay) ──┼── gpudriver  (/run/nvidia/driver symlink)
                     ...
                     └── rdmaplugin     (validate; Run→ re-assertion loop)
